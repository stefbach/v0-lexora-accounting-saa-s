...
export function RefineChat({
  text, domaines, onUpdate, placeholder, title,
  ...
  const resolvedTitle = title ?? t('scjur.refine.default_title', locale)
  ...
  async function apply() {
    ...
    try {
      ...
      if (!res.ok || !data) { alert((data && data.error) || t('scjur.refine.error_change', locale)); return }
      onUpdate(data.text || text, Array.isArray(data.sources) ? data.sources : [])
      setLog((l) => [...l, instruction])
      setInput("")
    } catch (e: any) {
      alert(e.message || t('scjur.refine.error_network', locale))
  ...
  return (
    <div className="rounded-2xl bg-white border border-gray-100 shadow-sm p-5">
      <p className="text-sm font-semibold flex items-center gap-2 mb-1" style={{ color: NAVY }}>
        <Settings className="w-4 h-4" style={{ color: GOLD }} /> {resolvedTitle}
      </p>
      <p className="text-xs text-gray-500 mb-3">{t('scjur.refine.help', locale)}</p>
      {log.length > 0 && (
      ...
      <div className="flex items-end gap-2">
        <textarea
          ...
          rows={1}
          disabled={!text}
          placeholder={text ? (placeholder || t('scjur.refine.placeholder_default', locale)) : t('scjur.refine.placeholder_disabled', locale)}
