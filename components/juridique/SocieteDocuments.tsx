...
export function SocieteDocuments({
  societeId, categorie, title = "Documents associés", hint,
}: {
  societeId?: string
  categorie: string
  title?: string
  ...
  async function upload(files: FileList | null) {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        if (!res.ok) { const e = await res.json().catch(() => ({})); alert(`Échec « ${file.name} » : ${e.error || res.status}`) }
      }
      await load()
  ...
  async function remove(path: string) {
    if (!societeId || !confirm("Supprimer ce document ?")) return
    ...
    <div className="rounded-2xl bg-white border border-gray-100 shadow-sm overflow-hidden">
      <div className="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
        <p className="font-bold text-sm flex items-center gap-2" style={{ color: NAVY }}>
          <FolderOpen className="w-4 h-4" style={{ color: "#D4AF37" }} /> {title} ({docs.length})
        </p>
        <button onClick={() => inputRef.current?.click()} disabled={uploading || !societeId}
          ...
          {uploading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <UploadCloud className="w-3.5 h-3.5" />} Associer un document
        </button>
      ...
      {loading ? (
        ...
        <p className="px-5 py-6 text-center text-sm text-gray-400">Aucun document associé. Importez vos justificatifs (PDF, images, Word).</p>
      ) : (
        <ul className="divide-y divide-gray-50">
          {docs.map((d) => (
            <li key={d.path} className="px-5 py-2.5 flex items-center gap-3">
              ...
              <button onClick={() => remove(d.path)} className="p-1.5 text-gray-300 hover:text-red-500" aria-label="Supprimer"><Trash2 className="w-4 h-4" /></button>
            </li>
          ))}
        </ul>
      )}
