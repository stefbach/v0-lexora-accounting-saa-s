...
// i18n: ce composant peut être rendu côté serveur (route API export-pdf) sans
// contexte navigateur, donc getLocale() renverrait toujours 'fr'. La locale est
// passée en PROP par l'appelant (défaut 'fr'). Labels statiques ci-dessous —
// parité avec le chunk cbil.glpdf.* de lib/i18n/client_bilan.ts.
type PdfLocale = 'fr' | 'en'
const GL_PDF_LABELS: Record<PdfLocale, Record<string, string>> = {
  fr: {
    title: 'GRAND LIVRE', subtitle: 'General Ledger',
    societe: 'SOCIÉTÉ', brn: 'BRN', periode: 'PÉRIODE', generated_on: 'GÉNÉRÉ LE',
    accounts: 'COMPTES', entries: 'ÉCRITURES',
    date: 'Date', jnl: 'Jnl', piece: 'N° Pièce', libelle: 'Libellé', lettre: 'Lettre',
    debit: 'Débit', credit: 'Crédit', solde: 'Solde',
    total: 'Total', grand_total_suffix: 'comptes', grand_total_prefix: 'TOTAL GÉNÉRAL',
    footer_generated: 'Généré par LEXORA',
    period_range_join: 'au', period_from: 'À partir du', period_until: 'Jusqu\'au', period_all: 'Toutes périodes',
  },
  en: {
    title: 'GENERAL LEDGER', subtitle: 'General Ledger',
    societe: 'COMPANY', brn: 'BRN', periode: 'PERIOD', generated_on: 'GENERATED ON',
    accounts: 'ACCOUNTS', entries: 'ENTRIES',
    date: 'Date', jnl: 'Jnl', piece: 'Doc No.', libelle: 'Description', lettre: 'Match',
    debit: 'Debit', credit: 'Credit', solde: 'Balance',
    total: 'Total', grand_total_suffix: 'accounts', grand_total_prefix: 'GRAND TOTAL',
    footer_generated: 'Generated by LEXORA',
    period_range_join: 'to', period_from: 'From', period_until: 'Until', period_all: 'All periods',
  },
}
...
export function GrandLivrePDF({ societe, dateDebut, dateFin, ecritures, compteNames, locale = 'fr' }: GrandLivrePDFProps) {
  const L = GL_PDF_LABELS[locale] ?? GL_PDF_LABELS.fr
  ...
  const periode     = dateDebut && dateFin
    ? `${fmtDate(dateDebut)} ${L.period_range_join} ${fmtDate(dateFin)}`
    : dateDebut ? `${L.period_from} ${fmtDate(dateDebut)}`
    : dateFin   ? `${L.period_until} ${fmtDate(dateFin)}`
    : L.period_all
  ...
  const TH = () => (
    <View style={S.hdrRow}>
      <View style={COL.date}><Text style={S.hdrTxt}>{L.date}</Text></View>
      <View style={COL.jnl}><Text style={S.hdrTxt}>{L.jnl}</Text></View>
      <View style={COL.piece}><Text style={S.hdrTxt}>{L.piece}</Text></View>
      <View style={COL.libelle}><Text style={S.hdrTxt}>{L.libelle}</Text></View>
      <View style={COL.lettre}><Text style={S.hdrTxt}>{L.lettre}</Text></View>
      <View style={COL.debit}><Text style={S.hdrTxt}>{L.debit}</Text></View>
      <View style={COL.credit}><Text style={S.hdrTxt}>{L.credit}</Text></View>
      <View style={COL.solde}><Text style={S.hdrTxt}>{L.solde}</Text></View>
    </View>
  )
      ...
      {pages.map((pg, pi) => (
        <Page key={pi} size="A4" style={S.page}>
          ...
          {pi === 0 && (
            <>
              ...
              <View style={{ height: 50, alignItems: 'center', justifyContent: 'center' }}>
                <Text style={{ fontSize: 16, fontWeight: 'bold' }}>{L.title}</Text>
              </View>
              <View style={{ height: 18, alignItems: 'center', justifyContent: 'center' }}>
                <Text style={{ fontSize: 10, color: '#555' }}>{L.subtitle} — {societe?.nom || '—'}</Text>
              </View>
              ...
              <View style={{ flexDirection: 'row', paddingTop: 12, paddingBottom: 12, marginTop: 10, marginBottom: 18, borderTopWidth: 0.5, borderTopColor: '#cccccc', borderBottomWidth: 0.5, borderBottomColor: '#cccccc' }}>
                <View style={{ width: '33.33%', paddingLeft: 8, paddingRight: 8 }}>
                  <View style={{ marginBottom: 4 }}><Text style={{ fontSize: 7, color: '#888' }}>{L.societe}</Text></View>
                  <View style={{ marginBottom: 10 }}><Text style={{ fontSize: 10, fontWeight: 'bold' }}>{societe?.nom || '—'}</Text></View>
                  <View style={{ marginBottom: 4 }}><Text style={{ fontSize: 7, color: '#888' }}>{L.brn}</Text></View>
                  <View><Text style={{ fontSize: 9 }}>{societe?.brn || '—'}</Text></View>
                </View>
                <View style={{ width: '33.33%', paddingLeft: 8, paddingRight: 8 }}>
                  <View style={{ marginBottom: 4 }}><Text style={{ fontSize: 7, color: '#888' }}>{L.periode}</Text></View>
                  <View style={{ marginBottom: 10 }}><Text style={{ fontSize: 10, fontWeight: 'bold' }}>{periode}</Text></View>
                  <View style={{ marginBottom: 4 }}><Text style={{ fontSize: 7, color: '#888' }}>{L.generated_on}</Text></View>
                  <View><Text style={{ fontSize: 9 }}>{today}</Text></View>
                </View>
                <View style={{ width: '33.33%', paddingLeft: 8, paddingRight: 8 }}>
                  <View style={{ marginBottom: 4 }}><Text style={{ fontSize: 7, color: '#888' }}>{L.accounts}</Text></View>
                  <View style={{ marginBottom: 10 }}><Text style={{ fontSize: 10, fontWeight: 'bold' }}>{groups.length}</Text></View>
                  <View style={{ marginBottom: 4 }}><Text style={{ fontSize: 7, color: '#888' }}>{L.entries}</Text></View>
            ...
            <View key={g.compte}>
              ...
              <View style={S.table}>
                ...
                <View style={S.rowTotal}>
                  ...
                  <View style={COL.libelle}><Text style={S.totalTxt}>{L.total} {g.compte}</Text></View>
          ...
          {pi === pages.length - 1 && (
            <View style={[S.table, { marginTop: 6 }]}>
              <View style={S.rowGrandTotal}>
                ...
                <View style={COL.libelle}><Text style={S.grandTxt}>{L.grand_total_prefix} — {groups.length} {L.grand_total_suffix}</Text></View>
          ...
          <View style={S.footer} fixed>
            <Text>{societe?.nom || '—'} | BRN: {societe?.brn || '—'} | {periode} | {L.footer_generated}</Text>
