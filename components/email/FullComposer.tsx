...
import { Mail, FileText, Loader2, Copy, CheckCircle, Download, Sparkles, AlertCircle, Wand2, Scale, X, Users } from "lucide-react"
...
const EXEMPLES = (locale: Loc) => [
  t('samsc.red_ex_relance', locale), t('samsc.red_ex_candidat', locale),
  t('samsc.red_ex_delai', locale), t('samsc.red_ex_convoc', locale),
]

/** Saisie destinataire avec autocomplétion contacts (Lexora + carnet Nylas). */
function RecipientField({ value, onChange, societeId, accountId, className, placeholder, id }: { value: string; onChange: (v: string) => void; societeId: string | null; accountId: string | null; className: string; placeholder?: string; id?: string }) {
  ...
  const search = async (q: string) => {
    ...
    try { const r = await fetch(`/api/nylas/contacts?${p.toString()}`); const d = await r.json(); setSuggestions(d.contacts || []); setOpen(true) } catch { /* noop */ }
  }
  return (
    <div className="relative">
      <input id={id} type="email" value={value} onChange={(e) => { onChange(e.target.value); search(e.target.value) }}
        onFocus={() => suggestions.length && setOpen(true)} onBlur={() => setTimeout(() => setOpen(false), 150)}
...
/** Composeur plein écran — reprend l'intégralité de l'assistant de rédaction,
 *  branché sur les contacts et l'envoi via la boîte connectée. */
export function FullComposer({ societeId, accountId, onClose }: { societeId: string | null; accountId: string | null; onClose: () => void }) {
  const locale = getLocale()
  const domaines = DOMAINES(locale), tons = TONS(locale), longueurs = LONGUEURS(locale), langues = LANGUES(locale), exemples = EXEMPLES(locale)
  const [mode, setMode] = useState<'email' | 'courrier'>('email')
  const [brief, setBrief] = useState(""); const [domaine, setDomaine] = useState('general')
  const [ton, setTon] = useState(tons[0]); const [longueur, setLongueur] = useState('moyen'); const [langue, setLangue] = useState('fr')
  const [objet, setObjet] = useState(""); const [expNom, setExpNom] = useState(""); const [expContact, setExpContact] = useState("")
  const [destNom, setDestNom] = useState(""); const [destAddr, setDestAddr] = useState(""); const [signataire, setSignataire] = useState("")
  const [loading, setLoading] = useState(false); const [pdfLoading, setPdfLoading] = useState(false)
  const [error, setError] = useState<string | null>(null); const [result, setResult] = useState(""); const [sources, setSources] = useState<Source[]>([])
  const [copied, setCopied] = useState(false)
  const [destEmail, setDestEmail] = useState(""); const [sending, setSending] = useState(false); const [sent, setSent] = useState(false); const [attachPdf, setAttachPdf] = useState(false)
  ...
  const toPlain = (s: string) => s.replace(/\*\*([^*]+)\*\*/g, "$1").replace(/\*([^*\n]+)\*/g, "$1")
  ...
  async function generate() {
    ...
    try {
      const res = await fetch("/api/redaction", { method: "POST", headers: { "Content-Type": "application/json" },
  ...
  async function copy() {
    ...
    } catch { try { await navigator.clipboard.writeText(plain); setCopied(true); setTimeout(() => setCopied(false), 1800) } catch { /* noop */ } }
  }
  ...
  async function downloadPdf() {
    ...
    setPdfLoading(true)
    try {
      const res = await fetch("/api/redaction/pdf", { method: "POST", headers: { "Content-Type": "application/json" },
  ...
  async function sendByEmail() {
    ...
    try {
      const res = await fetch("/api/redaction/send", { method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ societe_id: societeId, nylas_account_id: accountId || undefined, to: destEmail.trim(), subject: objet || destNom || "Message", body: result,
    ...
    } catch (e: any) { setError(e instanceof Error ? e.message : t('samsc.red_send_error', locale)) } finally { setSending(false) }
  }
    ...
    <div className="fixed inset-0 z-50 bg-gray-50 overflow-y-auto">
      <div className="max-w-5xl mx-auto p-4 md:p-6">
        ...
        <div className="flex items-center gap-3 mb-5">
          <div className="w-11 h-11 rounded-xl flex items-center justify-center" style={{ backgroundColor: NAVY }}><Wand2 className="w-5 h-5" style={{ color: GOLD }} /></div>
          <div className="flex-1">
            <h1 className="text-xl font-bold" style={{ color: NAVY }}>Composer un email</h1>
            <p className="text-xs text-gray-500">Assistant de rédaction — email ou courrier, avec sources juridiques/fiscales.</p>
          </div>
          <button onClick={onClose} aria-label="Fermer" className="w-11 h-11 rounded-xl flex items-center justify-center border border-gray-200 bg-white text-gray-600 hover:border-gray-300 hover:text-[#0B0F2E] transition-all shrink-0"><X className="w-5 h-5" /></button>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-5 items-start">
          ...
          <div className="space-y-4">
            <div className="rounded-2xl bg-white border border-gray-100 p-5 shadow-sm space-y-4">
              {/* Destinataire — toujours visible, avec autocomplétion contacts. */}
              {mode === 'email' && (
                <div>
                  <label className="text-xs font-semibold text-gray-600 flex items-center gap-1.5"><Users className="w-3 h-3" /> À (destinataire)</label>
                  <RecipientField value={destEmail} onChange={setDestEmail} societeId={societeId} accountId={accountId} className={field} placeholder="Nom ou email — carnet d'adresses + contacts Lexora" />
                </div>
              )}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5 mt-2">
                  {exemples.map((ex) => <button key={ex} onClick={() => setBrief(ex)} className="text-[11px] px-2 py-1 rounded-full border border-gray-200 text-gray-500 hover:border-[#D4AF37] hover:text-[#8a6d15] text-left">{ex.length > 52 ? ex.slice(0, 52) + '…' : ex}</button>)}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div><label className="text-xs font-semibold text-gray-600">{t('samsc.red_ton_label', locale)}</label><select value={ton} onChange={(e) => setTon(e.target.value)} className={field}>{tons.map((tn) => <option key={tn} value={tn}>{tn}</option>)}</select></div>
                <div><label className="text-xs font-semibold text-gray-600">{t('samsc.red_len_label', locale)}</label><select value={longueur} onChange={(e) => setLongueur(e.target.value)} className={field}>{longueurs.map((l) => <option key={l.id} value={l.id}>{l.label}</option>)}</select></div>
                <div><label className="text-xs font-semibold text-gray-600">{t('samsc.red_lang_label', locale)}</label><select value={langue} onChange={(e) => setLangue(e.target.value)} className={field}>{langues.map((l) => <option key={l.id} value={l.id}>{l.label}</option>)}</select></div>
              </div>

              <div><label className="text-xs font-semibold text-gray-600">{t('samsc.red_objet_label', locale)}</label><input value={objet} onChange={(e) => setObjet(e.target.value)} className={field} placeholder={t('samsc.red_objet_ph', locale)} /></div>
            ...
            <RefineChat text={result} endpoint="/api/redaction/refine" extraPayload={{ mode, langue }} onUpdate={(t) => setResult(t)} title={t('samsc.red_refine_title', locale)} placeholder={t('samsc.red_refine_ph', locale)} />
          </div>
          ...
          <div className="lg:sticky lg:top-4">
            <div className="rounded-2xl bg-white border border-gray-100 shadow-sm min-h-[400px]">
              ...
              <div className="p-5">
                ...
                {result && (
                  <>
                    <div className="whitespace-pre-wrap break-words font-sans text-[13px] leading-relaxed text-gray-800 max-h-[50vh] overflow-y-auto" dangerouslySetInnerHTML={{ __html: toBoldHtml(result) }} />
                    {sources.length > 0 && (
                    ...
                    {mode === 'email' && (
                      <div className="mt-4 pt-3 border-t border-gray-100 space-y-2">
                        <p className="text-[11px] font-semibold uppercase tracking-wider text-gray-400 flex items-center gap-1.5"><Users className="w-3.5 h-3.5" style={{ color: GOLD }} /> {t('samsc.red_send_title', locale)}</p>
                        <RecipientField id="redaction-dest-email" value={destEmail} onChange={setDestEmail} societeId={societeId} accountId={accountId} className={field} placeholder={t('samsc.red_send_to_ph', locale)} />
