        ...
        <div className="absolute z-20 mt-1 w-full bg-white border border-gray-200 rounded-xl shadow-lg max-h-52 overflow-y-auto">
          {suggestions.map((s) => (
            <button key={s.email} type="button" onMouseDown={(e) => { e.preventDefault(); onChange(s.email); setOpen(false) }}
              ...
              <span className="text-[10px] text-gray-400 shrink-0">{s.source === 'lexora' ? 'Lexora' : 'Carnet'}</span>
            </button>
          ))}
        </div>
      )}
  ...
  async function sendByEmail() {
    ...
    try {
      const res = await fetch("/api/redaction/send", { method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ societe_id: societeId, account_id: accountId || undefined, to: destEmail.trim(), subject: objet || destNom || "Message", body: result,
