...
| **`devise_origine`** | TEXT nullable | mig 207 — devise de la tx source (EUR/USD/MUR) |
| **`montant_origine`** | NUMERIC nullable | mig 207 — montant dans la devise d'origine |
| **`taux_change_applique`** | NUMERIC nullable | mig 207 — taux gelé au moment de l'écriture (pas live) |
...
**Contraintes** :
- `chk_ecritures_v2_numero_compte_format` (NOT VALID) : `numero_compte ~ '^[1-8][0-9]{2,4}$'` (mig 201)
...
**Triggers** : `trg_auto_verrouille_bulletin` (bloque modif si statut>=valide), `trig_ecritures_paie` DÉSACTIVÉ (mig 204).
...
- **`source_releve_id`**, **`source_transaction_idx`**, **`source_kind`** (mig 202) — tracking de la tx bancaire source
...
**Gotchas** :
- ⚠ Les mouvements créés avant mig 202 n'ont pas de `source_releve_id` → pas protégés par l'index. Nettoyables via `/admin/repair` action `purge_cca_doublons`.
...
**Migration** : `206_taux_change_historique.sql`.
