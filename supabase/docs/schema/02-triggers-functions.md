...
### `tr_00_legacy_3digit_warn` (mig 200, anciennement 162)
...
| Migration | `200_trigger_warn_legacy_3digit.sql` |
...
### `tr_balance_check_insert` + `tr_balance_check_update` (mig 201+168)
...
`devise_origine`, `montant_origine`) ajoutées par mig 207. Si tu INSERT via
...
### `trig_ecritures_paie` (⚠ DÉSACTIVÉ — mig 204)
...
| Ré-écrite | mig 029 (dual V1/V2), mig 120 (V2-only), mig 198 (PCM 4-digits) |
...
| Statut appel auto | Le trigger `trig_ecritures_paie` qui l'appelle est DÉSACTIVÉ (mig 204). Appelable manuellement en RPC via `supabase.rpc('generer_ecritures_paie', {p_bulletin_id})`. |
...
`devise_origine`, `montant_origine` (mig 207) sont ignorées.
