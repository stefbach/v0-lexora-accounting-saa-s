...
- Les codes **3-digits bare** (`421`, `431`, `432`, `433`, `444`) sont LEGACY → remappés automatiquement par `tr_00_legacy_3digit_warn` (mig 200).
...
## Plan comptable PCM Maurice (seed mig 201 + extensions 158/170)
...
**Enforcement** : triggers `tr_balance_check_insert` + `tr_balance_check_update` (mig 201+168) → `RAISE WARNING` non bloquant. Exclusions : `ref_folio LIKE 'BANK-%'` (paiements groupés 1:N) et `journal IN ('CLS', 'BNQ')` (classifications auto).
...
- `ux_mouvements_cca_source` (mig 202)
...
| R04_NPF_NSF | 41 | `npf\|nsf` | **`mauritius revenue\|mra`** (restreint mig 205) | 4312 | 512 | ✓ |
...
| **R05_INTERCO** | 50 | NULL | NULL | 580 | 512 | ⚠ **DISABLED** (mig 205) |
...
| **OD-PAIE** | Opérations paie par bulletin | Pipeline désactivé (mig 204). Similar à SAL mais par bulletin via RPC `generer_ecritures_paie` |
