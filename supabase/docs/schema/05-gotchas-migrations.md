...
- Erreur rencontrée mig 201 : `transition tables cannot be specified for triggers with column lists`.
- Fix mig 203 : séparer en **2 fonctions trigger** distinctes — une pour INSERT (sans OF), une pour UPDATE (sans OF). Chacune référence sa propre transition table.
...
- Le Grand Livre est **toujours** en MUR. Les colonnes `montant_devise`/`devise_origine`/`taux_change_applique` (mig 207) conservent la trace.
...
### Génération "refonte bancaire" (mig 196-172)
...
| **166** | `plan_comptable_strict_canonique` | Trigger strict pour canoniser à l'INSERT (tentative avec transition tables) | Erreur 0A000 — re-fixé mig 203 |
...
**Aucune des migrations 196-207 n'est trivialement réversible.** Elles modifient des données historiques (purges, backfills).
