...
const TOOLS = [
  ...
  },
  {
    name: 'audit_pcm',
    description:
      'Audite la conformité du PCM d\'une société. Retourne errors (comptes obligatoires manquants, écritures sur comptes archivés ou hors PCM), warnings (sous-comptes orphelins), suggestions (comptes inutilisés), et stats. Lecture seule.',
    inputSchema: {
      type: 'object' as const,
      properties: { societe_id: { type: 'string' } },
      required: ['societe_id'],
  ...
  },
  {
    ...
    description:
      'Liste les modules PCM actuellement activés sur une société (template_code, version, date activation).',
    inputSchema: {
      type: 'object' as const,
      properties: { societe_id: { type: 'string' } },
      required: ['societe_id'],
  ...
  },
  {
    ...
    description:
      'Active un module PCM sur une société (module_gbc1, module_holding, module_b2b_tech, module_health_clinic). Vérifie les prérequis (core_maurice requis). Idempotent. MODIFICATION — confirmation 2-step.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        societe_id: { type: 'string' },
  ...
  // ============================================================
  {
    ...
    description:
      'Liste les écritures du grand livre d\'une société. Filtres : compte, date_debut, date_fin, journal, lettre, unlettered_only. Retourne aussi les totaux débit/crédit.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        societe_id: { type: 'string' },
  ...
  },
  {
    ...
    description:
      'Balance des comptes du grand livre (débit/crédit/solde par compte) sur une période, optionnellement filtrée par classe. Vérifie l\'équilibre global.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        societe_id: { type: 'string' },
  ...
  },
  {
    ...
    description:
      'Crée une écriture comptable équilibrée (somme débits = somme crédits). MODIFICATION — confirmation 2-step. Les comptes doivent exister dans le PCM de la société.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        societe_id: { type: 'string' },
  ...
  },
  {
    ...
    description:
      'Reclasse les écritures d\'un compte vers un autre. TOUJOURS faire un dry_run=true d\'abord pour prévisualiser (nb écritures, totaux, échantillon), puis dry_run=false pour exécuter. Filtre optionnel par dates, journal, libellé.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        societe_id: { type: 'string' },
  ...
  },
  {
    ...
    description:
      'Lettre un ensemble d\'écritures avec un code commun. Vérifie l\'équilibre (débit=crédit) sauf si force_desequilibre. MODIFICATION — confirmation 2-step.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        societe_id: { type: 'string' },
  ...
  },
  {
    ...
    description:
      'Génère la déclaration TVA MRA (format EBS) pour une période : Output Tax (TVA collectée), Input Tax (TVA déductible), Net VAT à payer ou crédit. Basé sur les comptes tagués mra_ebs_output/input du PCM.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        societe_id: { type: 'string' },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      ...
      case 'audit_pcm': {
        const data = await lexoraFetch(
          `/api/societes/${encodeURIComponent(String(a.societe_id))}/pcm/audit`,
      ...
      case 'list_modules_actifs': {
        const data = await lexoraFetch(
          `/api/societes/${encodeURIComponent(String(a.societe_id))}/pcm/modules/activate`,
        )
        return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] }
      }

      case 'activate_module': {
        const guard = confirmGuard('activate_module', a,
          `Activer module ${a.module_code} sur société ${a.societe_id}`)
        if (!guard.confirmed) return guard.response
        const data = await lexoraFetch(
          `/api/societes/${encodeURIComponent(String(a.societe_id))}/pcm/modules/activate`,
          { method: 'POST', body: JSON.stringify({ module_code: a.module_code }) },
        )
        return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] }
      }
      ...
      case 'list_grand_livre': {
        ...
        if (a.date_fin) params.set('date_fin', String(a.date_fin))
        if (a.journal) params.set('journal', String(a.journal))
        if (a.lettre) params.set('lettre', String(a.lettre))
        if (a.unlettered_only) params.set('unlettered_only', 'true')
        if (a.limit !== undefined) params.set('limit', String(a.limit))
        const data = await lexoraFetch(
          `/api/societes/${encodeURIComponent(String(a.societe_id))}/grand-livre?${params}`,
        )
        return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] }
      }
      ...
      case 'get_balance_grand_livre': {
        ...
        if (a.date_debut) params.set('date_debut', String(a.date_debut))
        if (a.date_fin) params.set('date_fin', String(a.date_fin))
        if (a.classe) params.set('classe', String(a.classe))
        const data = await lexoraFetch(
          `/api/societes/${encodeURIComponent(String(a.societe_id))}/grand-livre/balance?${params}`,
        )
        return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] }
      }

      case 'create_journal_entry': {
        const guard = confirmGuard('create_journal_entry', a,
          `Créer écriture ${a.journal || 'OD'} "${a.libelle}" (${(a.lignes as unknown[] | undefined)?.length || 0} lignes) société ${a.societe_id}`)
        if (!guard.confirmed) return guard.response
        const data = await lexoraFetch(
          `/api/societes/${encodeURIComponent(String(a.societe_id))}/grand-livre`,
          {
            method: 'POST',
            body: JSON.stringify({
      ...
      case 'reclass_ecritures': {
        ...
        if (isExecute) {
          const guard = confirmGuard('reclass_ecritures', a,
            `EXÉCUTER reclassement ${a.from_compte} → ${a.to_compte} société ${a.societe_id}`)
          if (!guard.confirmed) return guard.response
        }
        const data = await lexoraFetch(
          `/api/societes/${encodeURIComponent(String(a.societe_id))}/grand-livre/reclass`,
          {
            method: 'POST',
            body: JSON.stringify({
      ...
      case 'lettrer_ecritures': {
        const guard = confirmGuard('lettrer_ecritures', a,
          `Lettrer ${(a.ecritures_ids as unknown[] | undefined)?.length || 0} écritures société ${a.societe_id}`)
        if (!guard.confirmed) return guard.response
        const data = await lexoraFetch(
          `/api/societes/${encodeURIComponent(String(a.societe_id))}/grand-livre/lettrage`,
          {
            method: 'POST',
            body: JSON.stringify({
