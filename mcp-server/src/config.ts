...
export type ConfigResult =
  | { ok: true; config: LexoraConfig }
  | { ok: false; errors: string[] }

const HELP_KEY = 'Génère-en une depuis Lexora → Direction → Connecter à Claude Desktop'

export function resolveLexoraConfig(env: Record<string, string | undefined>): ConfigResult {
  ...
  if (rawUrl.startsWith('lex_')) {
    errors.push(
      'LEXORA_API_URL contient une clé API ("lex_...") au lieu d\'une adresse. ' +
