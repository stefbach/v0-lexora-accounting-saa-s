...
export default function DocumentDetailClient({ docId, initialDocument, initialError }: DocumentDetailClientProps) {
  const locale = getLocale()
  const TYPE_LABELS = getTypeLabels(locale)
  const router = useRouter()

  const [doc, setDoc] = useState<DocumentDetail | null>(initialDocument)
  ...
  // Edit form state
  const [editTypeDoc, setEditTypeDoc] = useState(initialDocument?.type_document || "")
  ...
  useEffect(() => {
    // Load societes for reassignment (client-side only — needs auth context)
