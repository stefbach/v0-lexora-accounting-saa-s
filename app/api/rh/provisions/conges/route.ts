/**
 * GET /api/rh/provisions/conges?societe_id=&annee= — sprint G8 Phase 1.
 *
 * Liste des snapshots historiques. Auth : admin + rh.
 */
import { NextResponse } from 'next/server'
import { apiError } from '@/lib/api-error'
import { createClient } from '@supabase/supabase-js'
import { createClient as createServerClient } from '@/lib/supabase/server'
import { getSnapshotsSociete } from '@/lib/rh/ias19-provisions'
...
export async function GET(request: Request) {
  try {
    ...
    const snapshots = await getSnapshotsSociete(supabase, societeId, annee)
