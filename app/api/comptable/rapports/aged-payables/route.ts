...
// ---------------------------------------------------------------------------
// GET /api/comptable/rapports/aged-payables
// ---------------------------------------------------------------------------
...
export async function GET(request: Request) {
  try {
    ...
    // Alertes — spécifique aux dettes fournisseurs
    const alertes: Alerte[] = []
    if (nbOver90 > 0) {
      alertes.push({
        type: 'retard_paiement_critique',
        montant: Math.round(montantOver90 * 100) / 100,
        nb_factures: nbOver90,
        message: `Retards > 90j sur ${nbOver90} facture${nbOver90 > 1 ? 's' : ''} fournisseur${nbOver90 > 1 ? 's' : ''} — risque rupture`,
      })
    }
