...
const STATUT_STYLE: Record<string, { bg: string; c: string; label: string }> = {
  ouvert: { bg: "#EEF2FF", c: "#3730A3", label: "Ouvert" },
  en_cours: { bg: "#FEF9C3", c: "#854D0E", label: "En cours" },
  clos: { bg: "#F3F4F6", c: "#374151", label: "Clos" },
  gagne: { bg: "#ECFDF5", c: "#047857", label: "Gagné" },
  perdu: { bg: "#FEE2E2", c: "#B91C1C", label: "Perdu" },
  transige: { bg: "#E0F2FE", c: "#0369A1", label: "Transigé" },
}
  ...
  return (
    <div className="space-y-5">
      <JuridiqueHeader
        ...
        title="Dossiers"
        subtitle="Tous vos dossiers juridiques et contentieux, persistés et rattachés à leurs pièces, analyses et actes."
      />
        ...
        <>
          <div className="flex justify-end">
            <button onClick={() => setShowForm((v) => !v)} className="inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold text-white" style={{ background: NAVY }}>
              {showForm ? <X className="w-4 h-4" /> : <Plus className="w-4 h-4" />} {showForm ? "Annuler" : "Nouveau dossier"}
            </button>
          </div>
            ...
            <div className="rounded-2xl bg-white border border-gray-100 p-5 shadow-sm space-y-3">
              <div className="grid sm:grid-cols-2 gap-3">
                <div className="sm:col-span-2">
                  ...
                  <input value={form.intitule} onChange={(e) => setForm({ ...form, intitule: e.target.value })} placeholder="Ex. Acme Ltd c/ Nous — recouvrement facture #1234"
                ...
                <div>
                  ...
                  <input value={form.type_contentieux} onChange={(e) => setForm({ ...form, type_contentieux: e.target.value })} placeholder="recouvrement, travail…"
                    className="mt-1 w-full rounded-xl border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:border-[#D4AF37]" />
                </div>
                <div>
              ...
              <button onClick={create} disabled={saving || !form.intitule.trim()} className="inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-40" style={{ background: NAVY }}>
                {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Plus className="w-4 h-4" />} Créer le dossier
              </button>
            </div>
          )}
          ...
          <div className="rounded-2xl bg-white border border-gray-100 shadow-sm overflow-hidden">
            <div className="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
              <p className="font-bold text-sm" style={{ color: NAVY }}>Dossiers ({dossiers.length})</p>
              {loading && <Loader2 className="w-4 h-4 animate-spin text-gray-400" />}
            </div>
            {dossiers.length === 0 && !loading ? (
              <p className="px-5 py-8 text-center text-sm text-gray-400">Aucun dossier. Créez-en un.</p>
            ) : (
                  ...
                  return (
                    <li key={d.id}>
                      <Link href={`/juridique/dossiers/${d.id}`} className="px-5 py-3 flex items-center gap-3 hover:bg-gray-50">
                        ...
                        <span className="text-[11px] font-semibold px-2.5 py-1 rounded-full" style={{ background: st.bg, color: st.c }}>{st.label}</span>
