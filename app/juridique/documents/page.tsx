...
export default function DocumentsPage() {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        if (!res.ok) {
          ...
          alert(t("jurd.docs.uploadFail", locale).replace("{name}", file.name).replace("{error}", String(e.error || res.status)))
        }
      }
      await load()
  ...
  async function remove(path: string) {
    if (!societe?.id || !confirm(t("jurd.docs.removeConfirm", locale))) return
  ...
  return (
    <div className="space-y-5">
      <JuridiqueHeader
        ...
        title={t("jurd.docs.title", locale)}
        subtitle={t("jurd.docs.subtitle", locale)}
      />
      ...
      {!societe && !socLoading ? (
        <div className="rounded-2xl bg-white border border-gray-100 p-8 text-center text-sm text-gray-500">
          ...
          {t("jurd.docs.selectSociete", locale)}
        </div>
      ) : (
        <>
          {/* Zone d'upload */}
          ...
          >
            ...
            {uploading ? (
              <div className="flex flex-col items-center gap-2 text-sm text-gray-500">
                <Loader2 className="w-6 h-6 animate-spin" style={{ color: GOLD }} /> {t("jurd.docs.uploading", locale)}
              </div>
            ) : (
              <div className="flex flex-col items-center gap-2">
                ...
                <p className="text-sm font-semibold" style={{ color: NAVY }}>{t("jurd.docs.dropzone", locale)}</p>
                <p className="text-xs text-gray-400">{t("jurd.docs.dropzoneHint", locale)}</p>
              </div>
            )}
          </div>
          ...
          <div className="rounded-2xl bg-white border border-gray-100 shadow-sm overflow-hidden">
            <div className="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
              <p className="font-bold text-sm" style={{ color: NAVY }}>{t("jurd.docs.pieces", locale)} ({docs.length})</p>
              {loading && <Loader2 className="w-4 h-4 animate-spin text-gray-400" />}
            </div>
            {docs.length === 0 && !loading ? (
              <p className="px-5 py-8 text-center text-sm text-gray-400">{t("jurd.docs.empty", locale)}</p>
            ) : (
              <ul className="divide-y divide-gray-50">
                {docs.map((d) => (
                  <li key={d.path} className="px-5 py-3 flex items-center gap-3 hover:bg-gray-50">
                    ...
                    {d.url && (
                      <a href={d.url} target="_blank" rel="noreferrer" className="p-2 text-gray-400 hover:text-[#0B0F2E]" aria-label={t("jurd.docs.open", locale)}>
                        <ExternalLink className="w-4 h-4" />
                      </a>
                    )}
                    <button onClick={() => remove(d.path)} className="p-2 text-gray-400 hover:text-red-600" aria-label={t("jurd.docs.delete", locale)}>
