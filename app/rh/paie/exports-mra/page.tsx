...
export default function ExportsMRAPage() {
  ...
  const [banqueEmettrice, setBanqueEmettrice] = useState<string | null>(null)
  ...
  useEffect(() => {
    ...
    // Détecter la banque émettrice (compte bancaire de paie)
    setBanqueLoading(true)
    setBanqueEmettrice(null)
    fetch(`/api/comptable/comptes-bancaires?societe_id=${societe}`)
      .then(r => r.json())
      .then(d => {
        const comptes = d.comptes || d.data || []
        const comptePaie = comptes.find((c: any) => c.usage === 'paie' || c.usage === 'salaires') || comptes[0]
        setBanqueEmettrice(comptePaie ? (comptePaie.banque || comptePaie.nom || null) : null)
      })
      .catch(() => setBanqueEmettrice(null))
            ...
            <CardTitle className="text-base text-[#1E2A4A] flex items-center gap-2">
              ...
              {societe && (
                banqueLoading
                  ...
                  : banqueEmettrice
                  ? <span className="ml-2 px-2 py-0.5 bg-blue-100 text-blue-700 text-xs rounded-full font-medium">🏦 {banqueEmettrice}</span>
                  : <span className="ml-2 px-2 py-0.5 bg-yellow-100 text-yellow-700 text-xs rounded-full font-medium">⚠️ Compte non configuré</span>
              )}
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            {societe && !banqueLoading && !banqueEmettrice && (
              <div className="flex items-center gap-2 p-3 bg-yellow-50 border border-yellow-200 rounded-lg text-sm text-yellow-800">
                <AlertTriangle className="w-4 h-4 shrink-0" />
                <span>Configurez le compte bancaire de l'entreprise pour les virements de salaires.
                  <a href="/settings/banque" className="ml-1 underline font-medium">Configurer →</a>
                </span>
              </div>
            )}
            <div className="grid grid-cols-2 gap-4">
              <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border">
                ...
                <div className="flex flex-col gap-1 items-end">
                  ...
                  <a href="/rh/exports/virement" className="text-xs text-blue-600 underline">Voir page virements →</a>
                </div>
              </div>
              <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border">
                <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="p-4 bg-gray-50 rounded-lg border">
                ...
                <div className="text-xs text-gray-400 space-y-0.5">
                  ...
                  <p>• Salaire annualisé + TAN (fallback NIC)</p>
