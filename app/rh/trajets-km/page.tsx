...
function getStatutLabels(locale: Locale): Record<string, string> {
  return {
    en_cours: t('rhdiv.trajets.status_in_progress', locale),
    termine: t('rhdiv.trajets.status_to_validate', locale),
    valide: t('rhdiv.trajets.status_validated', locale),
    rejete: t('rhdiv.trajets.status_rejected', locale),
  }
}
...
function getVehiculeLabels(locale: Locale): Record<string, string> {
  return {
    voiture: t('rhdiv.trajets.car', locale),
    moto: t('rhdiv.trajets.moto', locale),
    velo: t('rhdiv.trajets.bike', locale),
  }
}
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2" style={{ color: NAVY }}>
            ...
            {t('rhdiv.trajets.title', locale)}
          </h1>
          <p className="text-gray-500 text-sm">{t('rhdiv.trajets.subtitle', locale)}</p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <Select value={societe} onValueChange={setSociete}>
            <SelectTrigger className="w-[200px]">
              <SelectValue placeholder={t('rhdiv.trajets.select_societe', locale)} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">{t('rhdiv.trajets.select_dash', locale)}</SelectItem>
              {societes.map(s => (
          ...
          <Select value={filterStatut} onValueChange={setFilterStatut}>
            ...
            <SelectContent>
              <SelectItem value="all">{t('rhdiv.trajets.all_status', locale)}</SelectItem>
              <SelectItem value="en_cours">{t('rhdiv.trajets.status_in_progress', locale)}</SelectItem>
              <SelectItem value="termine">{t('rhdiv.trajets.status_to_validate', locale)}</SelectItem>
              <SelectItem value="valide">{t('rhdiv.trajets.status_validated', locale)}</SelectItem>
              <SelectItem value="rejete">{t('rhdiv.trajets.status_rejected', locale)}</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </div>
      ...
      {societe === "all" && (
        <Card>
          <CardContent className="py-12 text-center text-gray-400">
            ...
            <p>{t('rhdiv.trajets.please_select', locale)}</p>
          </CardContent>
        </Card>
      )}
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <Card className="border-2" style={{ borderColor: BLUE }}>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-gray-500 flex items-center gap-2">
                  <Route className="h-4 w-4" /> {t('rhdiv.trajets.kpi_total_km', locale)}
                </CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card className="border-2" style={{ borderColor: GOLD }}>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-gray-500 flex items-center gap-2">
                  <DollarSign className="h-4 w-4" /> {t('rhdiv.trajets.kpi_total_amount', locale)}
                </CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-gray-500 flex items-center gap-2">
                  <TrendingUp className="h-4 w-4" /> {t('rhdiv.trajets.kpi_nb_trips', locale)}
                </CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => setShowParams(true)}>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-gray-500 flex items-center gap-2">
                  <Settings className="h-4 w-4" /> {t('rhdiv.trajets.kpi_rate_per_km', locale)}
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-1">
                <div className="flex items-center gap-2 text-sm">
                  <Car className="h-3 w-3" />
                  <span>{t('rhdiv.trajets.car', locale)}: {tauxVoitureVal != null ? `${tauxVoitureVal} Rs/km` : t('rhdiv.trajets.not_set', locale)}</span>
                </div>
                <div className="flex items-center gap-2 text-sm">
                  <Bike className="h-3 w-3" />
                  <span>{t('rhdiv.trajets.moto', locale)}: {tauxMotoVal != null ? `${tauxMotoVal} Rs/km` : t('rhdiv.trajets.not_set', locale)}</span>
                </div>
                <div className="flex items-center gap-2 text-xs text-gray-400">
                  <span>{t('rhdiv.trajets.click_edit', locale)}</span>
                </div>
              </CardContent>
            </Card>
          </div>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-lg" style={{ color: NAVY }}>{t('rhdiv.trajets.section_trips', locale)}</CardTitle>
            </CardHeader>
            <CardContent>
              {trajets.length === 0 ? (
                <p className="text-gray-400 text-sm text-center py-8">{t('rhdiv.trajets.no_trips', locale)}</p>
              ) : (
                <div className="overflow-x-auto">
                  <Table>
                    <TableHeader>
                      <TableRow>
                        <TableHead>{t('rhdiv.trajets.col_date', locale)}</TableHead>
                        <TableHead>{t('rhdiv.trajets.col_employee', locale)}</TableHead>
                        <TableHead>{t('rhdiv.trajets.col_vehicle', locale)}</TableHead>
                        <TableHead>{t('rhdiv.trajets.col_from', locale)}</TableHead>
                        <TableHead>{t('rhdiv.trajets.col_to', locale)}</TableHead>
                        <TableHead className="text-right">{t('rhdiv.trajets.col_distance', locale)}</TableHead>
                        <TableHead className="text-right">{t('rhdiv.trajets.col_amount', locale)}</TableHead>
                        <TableHead>{t('rhdiv.trajets.col_status', locale)}</TableHead>
                        <TableHead className="text-right">{t('rhdiv.trajets.col_actions', locale)}</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {trajets.map(tr => {
                        ...
                        return (
                          <TableRow key={tr.id} className="cursor-pointer hover:bg-gray-50" onClick={() => setSelectedTrajet(tr)}>
                            ...
                            <TableCell className="text-right" onClick={e => e.stopPropagation()}>
                              {tr.statut === "termine" && (
                                <div className="flex items-center gap-1 justify-end">
                                  <Button
                                    ...
                                    title={t('rhdiv.trajets.validate', locale)}
                                  >
                                  ...
                                  <Button
                                    ...
                                    title={t('rhdiv.trajets.reject', locale)}
                                  >
                              ...
                              {tr.statut === "valide" && (
                                <span className="text-green-600 text-xs font-medium">{t('rhdiv.trajets.status_validated', locale)}</span>
                              )}
                              {tr.statut === "rejete" && (
                                <span className="text-red-500 text-xs font-medium">{t('rhdiv.trajets.status_rejected', locale)}</span>
                              )}
                            </TableCell>
                          </TableRow>
                        )
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2" style={{ color: NAVY }}>
              ...
              {t('rhdiv.trajets.params_title', locale)}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 pt-2">
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <Label className="text-sm">{t('rhdiv.trajets.rate_car', locale)}</Label>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <Label className="text-sm">{t('rhdiv.trajets.rate_moto', locale)}</Label>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[11px] text-gray-400 mt-0.5">
                    {t('rhdiv.trajets.bike_hint', locale)}
                  </p>
                </div>
              </div>
            </div>
            <div className="border-t pt-3">
              <Label className="text-sm">{t('rhdiv.trajets.monthly_cap', locale)}</Label>
              <Input
                type="number"
                step="100"
              ...
              />
              <p className="text-xs text-gray-400 mt-1">{t('rhdiv.trajets.monthly_cap_hint', locale)}</p>
            </div>
            <Button
            ...
            >
              ...
              {t('rhdiv.trajets.save_params', locale)}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2" style={{ color: NAVY }}>
              ...
              {t('rhdiv.trajets.detail_title', locale)}
            </DialogTitle>
          </DialogHeader>
          {selectedTrajet && (
            <div className="space-y-4 pt-2">
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  <span className="text-gray-500">{t('rhdiv.trajets.lbl_employee', locale)}</span>
                  <p className="font-medium">{selectedTrajet.employe_prenom} {selectedTrajet.employe_nom}</p>
                </div>
                <div>
                  <span className="text-gray-500">{t('rhdiv.trajets.lbl_vehicle', locale)}</span>
                  <p className="font-medium">{getVehiculeLabels(locale)[selectedTrajet.vehicule_type] || selectedTrajet.vehicule_type}</p>
                </div>
                <div>
                  <span className="text-gray-500">{t('rhdiv.trajets.lbl_from', locale)}</span>
                ...
                <div>
                  <span className="text-gray-500">{t('rhdiv.trajets.lbl_to', locale)}</span>
                ...
                <div>
                  <span className="text-gray-500">{t('rhdiv.trajets.lbl_distance', locale)}</span>
                  <p className="text-lg font-bold" style={{ color: BLUE }}>{fmtKm(selectedTrajet.distance_km)}</p>
                </div>
                <div>
                  <span className="text-gray-500">{t('rhdiv.trajets.lbl_amount', locale)}</span>
                  <p className="text-lg font-bold" style={{ color: GOLD }}>{fmt(selectedTrajet.indemnite)}</p>
                  <p className="text-xs text-gray-400">{t('rhdiv.trajets.lbl_rate', locale).replace('{n}', String(selectedTrajet.taux_km))}</p>
                </div>
              </div>
              ...
              {selectedTrajet.steps && selectedTrajet.steps.length > 0 && (
                <div className="border-t pt-3">
                  <p className="text-sm font-semibold mb-2" style={{ color: NAVY }}>{t('rhdiv.trajets.steps', locale).replace('{n}', String(selectedTrajet.steps.length))}</p>
              ...
              {selectedTrajet.statut === "termine" && (
                <div className="flex gap-2 border-t pt-3">
                  ...
                  >
                    <CheckCircle className="h-4 w-4 mr-2" /> {t('rhdiv.trajets.validate', locale)}
                  </Button>
                  <Button
                    variant="outline"
                  ...
                  >
                    <XCircle className="h-4 w-4 mr-2" /> {t('rhdiv.trajets.reject', locale)}
                  </Button>
                </div>
              )}
            </div>
