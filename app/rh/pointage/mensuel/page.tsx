...
function calcOT(hEntree: string, hSortie: string, isFerie: boolean): { normales: number; ot15: number; ot2: number } {
  ...
  if (isFerie) return { normales: 0, ot15: 0, ot2: totalH }
...
export default function PointageMensuelPage() {
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      {recap && (
        <div className="grid grid-cols-6 gap-3">
          {[
            ...
            { label: "OT 1.5x", v: `${recap.total_ot_1_5x?.toFixed(1)}h = ${recap.montant_ot_1_5x?.toLocaleString("fr-FR")} MUR` },
            { label: "OT 2x", v: `${recap.total_ot_2x?.toFixed(1)}h = ${recap.montant_ot_2x?.toLocaleString("fr-FR")} MUR` },
                    ...
                    <TableRow key={i} className={getRowColor(row)}>
                      ...
                      <TableCell>
                        {weekend ? <span className="text-xs text-gray-400">Week-end</span>
                          ...
                          : hasOT ? <span className="text-xs text-orange-600 font-medium">OT ✓</span>
                      ...
                      <TableCell>
                        {!weekend && !ferie && (
                          <div className="flex gap-1 flex-wrap">
                            ...
                            {hasOT && !row.ot_valide && (
                              <Button size="sm" variant="outline" className="h-6 text-xs px-2 text-orange-700 border-orange-300" onClick={() => setOtDialog(row)}>⏱️ OT</Button>
                            )}
                            {!row.heure_entree && !row.absent_justifie && (
      ...
      {/* Dialog valider OT */}
      <Dialog open={!!otDialog} onOpenChange={open => !open && setOtDialog(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Valider les heures supplémentaires</DialogTitle>
          </DialogHeader>
          {otDialog && (() => {
            const ot = calcOT(otDialog.heure_entree, otDialog.heure_sortie, isFerie(otDialog.date))
            return (
              <div className="space-y-3 py-2">
                <p className="text-sm"><strong>{otDialog.employe?.prenom} {otDialog.employe?.nom}</strong> — {otDialog.date}</p>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setOtDialog(null)}>Annuler</Button>
            <Button onClick={validerOT} disabled={otSaving} className="bg-orange-600 text-white">
              {otSaving && <Loader2 className="w-4 h-4 animate-spin mr-2" />}✅ Valider les OT
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
